 ...
 *
 * LLM client for pure AI-agent diagnosis and decision making.
 * Strictly uses LLM tool-calling (Groq or Anthropic Claude) with structured JSON schemas.
 */
...
/**
 * Generates diagnosis using real LLM tool-calling.
 */
export async function generateDiagnosis(prompt: string): Promise<DiagnosisOutput> {
  ...
  if (groq) {
    ...
    logger.info({ model }, "Calling Groq LLM for Diagnosis...");
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert revenue recovery AI agent. Your sole job is to diagnose the root cause of a failed payment or revenue leak with structured precision.",
        },
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                reasoning: {
                  type: "string",
                  description: "A comprehensive explanation of the diagnosis and evidence.",
                },
              },
  ...
  // 2. Anthropic Claude
  if (anthropic) {
    logger.info("Calling Anthropic Claude for Diagnosis...");
...
/**
 * Generates recovery decision using real LLM tool-calling.
 */
export async function generateDecision(prompt: string): Promise<DecisionOutput> {
  ...
  if (groq) {
    const model = process.env["GROQ_MODEL"] || "openai/gpt-oss-120b";
    logger.info({ model }, "Calling Groq LLM for Decision...");
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert revenue recovery AI agent. Your sole job is to decide on the best recovery action and channel based on a case diagnosis and recovery policy.",
        },
        { role: "user", content: prompt },
      ],
      tools: [
            ...
            parameters: {
              ...
              properties: {
                ...
                reasoning: {
                  type: "string",
                  description:
                    "A detailed justification for the selected action and channel, verifying policy compliance.",
                },
              },
  ...
  // 2. Anthropic Claude
  if (anthropic) {
    logger.info("Calling Anthropic Claude for Decision...");
    const response = await anthropic.messages.create({
      model: "claude-3-5-sonnet-20241022",
      max_tokens: 1024,
      temperature: 0.1,
